...
def format(document, data):
    """Hàm chính định dạng cho loại văn bản Thông Báo."""
    print("Bắt đầu định dạng Thông báo...")
    title = data.get("title", "Thông báo về việc ABC")
    body = data.get("body", "Nội dung thông báo...") # Nội dung đã qua AI làm sạch
    ...
    # Thông báo thường không có gạch dưới trích yếu

    # 4. Nội dung chính (Thường là các đoạn văn bản thông thường)
    body_lines = body.split('\n')
    # Kiểm tra xem có dòng "Kính gửi" không
    has_kinh_gui = any(line.strip().upper().startswith("KÍNH GỬI") for line in body_lines)
    ...
    for line in body_lines:
        ...
        if stripped_line:
            ...
            # Kính gửi thì thụt đầu dòng bằng nội dung chính
            indent = FIRST_LINE_INDENT if is_kinh_gui else Cm(0)
            first_indent_val = Cm(0) if is_kinh_gui else FIRST_LINE_INDENT

            set_paragraph_format(p, alignment=align, left_indent=indent, first_line_indent=first_indent_val, line_spacing=1.5, space_after=Pt(6))
            add_run_with_format(p, stripped_line, size=FONT_SIZE_DEFAULT, bold=is_kinh_gui)

    # 5. Chữ ký
    format_signature_block(document, data)

    # 6. Nơi nhận (Thông báo có thể có hoặc không, nhưng vẫn thêm theo chuẩn)
